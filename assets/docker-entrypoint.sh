...
if [ "${1}" = "" ]; then
    ...
    if [ "${WIREMOCK_SECURED}" = "true" ]; then
        ...
        if [ ! "${WIREMOCK_CASTORE}" = "" ] && [ -f "${WIREMOCK_HOME}/conf/${WIREMOCK_CASTORE}" ]; then
            WIREMOCK_OPTS="${WIREMOCK_OPTS} --ca-keystore ${WIREMOCK_HOME}/conf/${WIREMOCK_CASTORE}"
        ...
        if [ ! "${WIREMOCK_TRUSTSTORE}" = "" ] && [ -f "${WIREMOCK_HOME}/conf/${WIREMOCK_TRUSTSTORE}" ]; then
            WIREMOCK_OPTS="${WIREMOCK_OPTS} --disable-http --https-port 8443 --https-truststore ${WIREMOCK_HOME}/conf/${WIREMOCK_TRUSTSTORE}"
            if [ ! "${WIREMOCK_TRUSTSTORE_PASSWORD}" = "" ]; then
    ...
    if [ -f "${WIREMOCK_HOME}/conf/log4j2.xml" ]; then
        JAVA_OPTS="${JAVA_OPTS} -Dlog4j.configurationFile=${WIREMOCK_HOME}/conf/log4j2.xml"
    else
        JAVA_OPTS="${JAVA_OPTS} -Dlog4j.configurationFile=/var/wiremock/conf/log4j2.xml"
    fi
